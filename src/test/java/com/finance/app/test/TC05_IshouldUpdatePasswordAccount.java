...
public class TC05_IshouldUpdatePasswordAccount extends TestCase {
	private boolean isPass;
	private FinanceModule financeModule;
	
	public TC05_IshouldUpdatePasswordAccount() {
		super("TC01 - I should update the password from edit account");
	}
	...
	public void testIshouldSignInLogin() {
		...
		financeModule.updatePasswordAccount(user);
		assertEquals("User updated a new password", true, isPass);
		TestResults.addResult("User updated a new password: "+TestResults.check(isPass));
		
		
		data = DataProperties.getProperties(BaseFinanceConstants.DATA_PROPERTIES);
		...
		assertEquals("User signed in in the login page with new password", true, isPass);
